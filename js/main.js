...
const App = {
    ...
    async init() {
        ...
        try {
            ...
            // Auto-update check status (non-blocking)
            Data.checkAutoStatusUpdates().then(function (result) {
                if (result && result.updated > 0) {
                    UI.showToast('info', 'Otomatik Guncelleme', result.updated + ' adet vadesi gecen cek otomatik olarak ODENDI isaretlendi.');
                    // Refresh UI if updates occurred
                    UI.render();
                    Charts.refresh();
                }
            }).catch(function (e) {
    ...
    bindEvents: function () {
        ...
        if (adminLoginForm) {
            adminLoginForm.addEventListener('submit', function (e) {
                UI.handleAdminLogin(e);
            });
        }

        // Admin password change form
        var adminPassForm = document.getElementById('admin-change-password-form');
        if (adminPassForm) {
            adminPassForm.addEventListener('submit', function (e) {
                UI.handleAdminPasswordChange(e);
            });
        }
        ...
        if (adminLogoutBtn) {
            adminLogoutBtn.addEventListener('click', function () {
                Auth.logoutAdmin();
                UI.closeAllModals();
                UI.showToast('info', 'Çıkış Yapıldı', 'Yönetici modundan çıkış yapıldı.');

                // Reset sidebar button text
                const label = document.querySelector('#admin-btn .nav-item-text');
                if (label) label.textContent = 'Yönetici Paneli';
            });
        }
