...
const Data = {
    ...
    /**
     * Bank Aliases - Standardization
     */
    bankAliases: {
        'NEARESTBANK': 'NEAR EAST BANK',
        'NEAR EAST': 'NEAR EAST BANK',
        'GARANTI': 'GARANTİ BANKASI'
    },
    ...
    /**
     * Get canonical bank name
     */
    getCanonicalBankName: function (name) {
        if (!name) return '';
        var upperName = name.trim().toUpperCase();
        return this.bankAliases[upperName] || upperName;
    },
    ...
    getBanks() {
        ...
        const allBanks = [...standardBanks, ...this.customBanks];
        const banks = new Set();

        // Add standard and custom banks (normalized)
        allBanks.forEach(bank => {
            banks.add(this.getCanonicalBankName(bank));
        });

        // Add banks from existing checks
        this.checks.forEach(check => {
            if (check.banka) {
                banks.add(this.getCanonicalBankName(check.banka));
            }
        });
    ...
    search: function (query, filters) {
        ...
        if (filters.bank) {
            ...
            results = results.filter((c) => {
                return this.getCanonicalBankName(c.banka) === bankFilter;
            });
        }
    ...
    /**
     * Check for auto status updates (vade tarihi < today - 1 day)
     */
    async checkAutoStatusUpdates() {
        if (!this.checks || this.checks.length === 0) return { updated: 0 };

        var today = new Date();
        today.setHours(0, 0, 0, 0);

        // Threshold date: Today minus 1 day (yesterday)
        ...
        for (var i = 0; i < this.checks.length; i++) {
            ...
            // If due date is BEFORE threshold (older than yesterday), mark as paid
            if (dueDate < thresholdDate) {
                console.log('Auto-updating check:', check.id, check.firma_adi, check.vade_tarihi);
                check.odeme_durumu = 'ÖDENDİ';
                check.updatedAt = new Date().toISOString();
                check.autoUpdated = true; // Flag to indicate auto update
                updatedCount++;
                hasChanges = true;
            }
        }
        ...
        if (hasChanges) {
            ...
            console.log('Auto-updated ' + updatedCount + ' checks to PAID status');
        }
