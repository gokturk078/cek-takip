...
const Email = {
     ...
     */
    async init() {
        // Store original defaults from config.js
        var defaults = {
        ...
        // Auto-correct old email if present in storage
        if (settings && settings.emailjs && settings.emailjs.TO_EMAIL === 'g.kahriman078@gmail.com') {
            console.log('Detected old email address, auto-correcting to:', defaults.TO_EMAIL);
            settings.emailjs.TO_EMAIL = defaults.TO_EMAIL;
            Utils.storage.set(CONFIG.SETTINGS_KEY, settings);
        }

        if (settings && settings.emailjs) {
            // Only use stored value if it's not empty, otherwise fall back to defaults
            CONFIG.EMAILJS.SERVICE_ID = settings.emailjs.SERVICE_ID || defaults.SERVICE_ID;
            CONFIG.EMAILJS.TEMPLATE_ID = settings.emailjs.TEMPLATE_ID || defaults.TEMPLATE_ID;
            CONFIG.EMAILJS.PUBLIC_KEY = settings.emailjs.PUBLIC_KEY || defaults.PUBLIC_KEY;
            CONFIG.EMAILJS.TO_EMAIL = settings.emailjs.TO_EMAIL || defaults.TO_EMAIL;
        } else {
            // No stored settings, use defaults
            CONFIG.EMAILJS.SERVICE_ID = defaults.SERVICE_ID;
            CONFIG.EMAILJS.TEMPLATE_ID = defaults.TEMPLATE_ID;
            CONFIG.EMAILJS.PUBLIC_KEY = defaults.PUBLIC_KEY;
            CONFIG.EMAILJS.TO_EMAIL = defaults.TO_EMAIL;
        }

        console.log('EmailJS Config:', {
            SERVICE_ID: CONFIG.EMAILJS.SERVICE_ID,
            TEMPLATE_ID: CONFIG.EMAILJS.TEMPLATE_ID,
            PUBLIC_KEY: CONFIG.EMAILJS.PUBLIC_KEY ? CONFIG.EMAILJS.PUBLIC_KEY.substring(0, 5) + '...' : 'MISSING',
