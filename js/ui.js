...
const UI = {
    ...
    renderTableRow(check) {
      ...
      <tr data-id="${check.id}" class="${rowClass}">
        ...
        <td class="actions-cell">
          <button class="action-btn edit admin-only" data-action="edit" data-id="${check.id}" title="Düzenle">
            <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
            </svg>
          </button>
          <button class="action-btn delete admin-only" data-action="delete" data-id="${check.id}" title="Sil">
            <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    openAdminLoginModal() {
        if (Auth.isAdmin) {
            // If already admin, open settings/password change directly or toggle
            // For now, let's open password change
    ...
    async handleAdminLogin(e) {
        ...
        const result = await Auth.attemptAdminLogin(password);

        if (result.success) {
            this.closeAllModals();
            this.showToast('success', 'Giriş Başarılı', 'Yönetici moduna hoş geldiniz.');
            this.render(); // Re-render to show admin buttons

            // Update sidebar button text/style if needed
            const label = document.querySelector('#admin-btn .nav-item-text');
            if (label) label.textContent = 'Yönetici (Aktif)';

        } else {
            this.showToast('danger', 'Hatalı Şifre', 'Lütfen tekrar deneyiniz.');
            const input = document.getElementById('admin-password-input');
            input.value = '';
            input.focus();
        }
    ...
    async handleAdminPasswordChange(e) {
        ...
        const result = await Auth.changeAdminPassword(currentPass, newPass);

        if (result.success) {
            this.closeAllModals();
            this.showToast('success', 'Başarılı', 'Yönetici şifresi güncellendi.');
        } else {
            this.showToast('danger', 'Hata', result.error || 'Şifre değiştirilemedi.');
        }
    },
